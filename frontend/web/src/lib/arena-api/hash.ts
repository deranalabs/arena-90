...
export function sha256Hex(value: string): string {
  ...
  view.setUint32(paddedLength - 8, Number(bitLength >> BigInt(32)), false);
  view.setUint32(paddedLength - 4, Number(bitLength & BigInt(0xffffffff)), false);

  const hash: number[] = [...initialState];
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choice + (roundConstants[index] as number) + (words[index] as number)) >>>
        0;
